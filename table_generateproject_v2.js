...
function TableDataShow(id,json,status) {
    ...
    if(indexjson == false)
    {
        ...
        DataTableShow_all.push(dataTableObject);
    }else{
       //移除原本的陣列表格
       let targetIndex = DataTableShow_all.findIndex(obj => obj.id === id);
       if (targetIndex !== -1) {
            // 移除陣列中的元素
            DataTableShow_all.splice(targetIndex, 1);
            // 移除後重新push
            let dataTableObject = {
                id: id,
                dataTable: DataTableFirst
    ...
    if(body)
    {
        body.addEventListener('click', (event)=>{
            ...
            switch(dataTable)
            {
                ...
                case "right_add":
                    //(1.) 先取得目前所在欄位
                    let thisTable = target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
                    let rad_Table = thisTable;
                    let trparent = rad_Table.querySelector('thead tr');
                    //(2.) 取得目前所在是第幾個
                    //取得所有th
                    ...
                    //生成表格
                    AddTableShow(rad_Table,thparents,liIndex);
                    break;
                //移除
                case "del_field":
                    //取得目前table id
                    let del_thisTable = target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
                    //取得push的datable的值
                    let indexTable_del = FindDatatable(del_thisTable.id);
                    let DataTableShow = indexTable_del[0].dataTable;
                    //取得目前是幾欄位
                    ...
                    else
                    {
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                ...
                                const liIndex = Array.prototype.indexOf.call(thparents, thparent);
                                //移除欄位
                                DelTableShow(del_thisTable,thparents,liIndex);
                            }
                        });
                    }
                ...
                case 'bottom_add':
                    //取得目前table id
                    let badd_thisTable = target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
                    //(1.)取得目前所在td位置
                    let tdposition = target.parentElement.parentElement.parentElement.parentElement;
                    //(2.)取得目前所在tr
                    let trposition = tdposition.parentElement;
                    //(3.) tr 是第幾個
                    let tbody = badd_thisTable.querySelector('tbody');
                ...
                case 'del_column':
                    //取得目前是幾欄位
                    //取得目前table id
                    let delc_thisTable = target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
                    //取得push的datable的值
                    let indexTable_delc = FindDatatable(delc_thisTable.id);
                    let DataTableShows = indexTable_delc[0].dataTable;
                    ...
                    else
                    {
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                ...
                                let trposition = tdposition.parentElement;
                                //(3.) tr 是第幾個
                                let tbody = delc_thisTable.querySelector('tbody');
                                let tbody_tr = tbody.querySelectorAll('tr');
                                const tr_index = Array.prototype.indexOf.call(tbody_tr, trposition);
                                //(4.) td 是第幾個
                                let tbody_td = trposition.querySelectorAll('td');
                                const td_index = Array.prototype.indexOf.call(tbody_td, tdposition);
                                //移除列
                                DelRowTableShow(delc_thisTable,tbody_tr,tr_index);
                            }
                        });
                    }
                    break;
                //向右合併
                case "colspan_merge":
                    let cmerge_thisTable = target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
                    let position_array = TDposition(cmerge_thisTable,target);
                    ColspanMerge(cmerge_thisTable,position_array);
                    break;
                //取消合併
                case "cancel_merge":
                    let cancelerge_thisTable = target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
                    let position_array2 = TDposition(cancelerge_thisTable,target);
                    CancelMerge(cancelerge_thisTable,position_array2);
                    break;
                //向下合併
                case "rowspan_merge":
                    let rowspan_thisTable = target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
                    let position_array4 = TDposition(rowspan_thisTable,target);
                    RowspanMerge(rowspan_thisTable,position_array4);
                    break;
            }
        });
    }
...
async function TableAddFieldRowTbody(Table,liIndex,position_array,status)
{
    ...
    if(indexjson)
    {
        row = indexjson[0].dataTable.rows().count();
    }
...
async function AddTableShow(Table,thparents,liIndex)
{
    ...
    let promise3 = await new Promise((resolve,reject)=>{
        ...
        if(demotest_start)
        {
            ...
            if(demotest == 1)
            {
                let index_index = FindDatatable(Table.id);
                if(index_index)
                {
                    let DataTableShow = index_index[0].dataTable;
                    DataTableShow.destroy();
                    TableDataShow(Table.id,newjson,'backend');
                    demotest = demotest - 1;
                    resolve('json');
                }
